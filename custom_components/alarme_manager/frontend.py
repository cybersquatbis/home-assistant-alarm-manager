...
async def async_register_frontend(hass):
    ...
    if PANEL_URL not in hass.data.get("frontend_panels",{}):
        await panel_custom.async_register_panel(hass=hass,frontend_url_path=PANEL_URL,webcomponent_name="alarme-manager-panel",module_url=f"{STATIC_URL}/alarme-manager.js",sidebar_title="Alarme Manager",sidebar_icon="mdi:shield-home",require_admin=False)
