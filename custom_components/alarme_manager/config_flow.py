    ...
    return vol.Schema(
        {
            ...
            vol.Optional(CONF_ALARM_ENTITY, default=defaults.get(CONF_ALARM_ENTITY)): _entity_selector(
                "alarm_control_panel"
            ),
            vol.Optional(CONF_OPENINGS, default=defaults.get(CONF_OPENINGS, [])): _entity_selector(
                multiple=True
            ),
            vol.Optional(CONF_MOTIONS, default=defaults.get(CONF_MOTIONS, [])): _entity_selector(
                multiple=True
            ),
            vol.Optional(CONF_CAMERAS, default=defaults.get(CONF_CAMERAS, [])): _entity_selector(
                "camera", multiple=True
            ),
            vol.Optional(CONF_LIGHTS, default=defaults.get(CONF_LIGHTS, [])): _entity_selector(
                "light", multiple=True
            ),
            vol.Optional(CONF_SIREN, default=defaults.get(CONF_SIREN)): _entity_selector(),
            vol.Optional(
                CONF_SIREN_DURATION,
                default=defaults.get(CONF_SIREN_DURATION, DEFAULT_SIREN_DURATION),
            ): selector.NumberSelector(
                selector.NumberSelectorConfig(min=1, max=300, step=1, mode=selector.NumberSelectorMode.BOX)
            ),
            vol.Optional(
                CONF_CRITICAL_ENTITIES, default=defaults.get(CONF_CRITICAL_ENTITIES, [])
            ): _entity_selector(multiple=True),
            vol.Optional(CONF_RF_ENTITIES, default=defaults.get(CONF_RF_ENTITIES, [])): _entity_selector(
                multiple=True
            ),
...
class AlarmManagerConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        """Create the single Community configuration."""
